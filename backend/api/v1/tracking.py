...
def register_socketio_handlers(socketio):
    ...
    def on_call_offer(data):
        ...
        try:
            ...
            # Native app wake-up fallback (Android FCM / iOS VoIP push) —
            # fires alongside the socket relay and store_sdp_offer above,
            # for the case where the callee's app is fully closed and can't
            # receive it any other way. No-ops harmlessly if the callee has
            # no registered native device token (e.g. browser/PWA-only users).
            if callee_id:
