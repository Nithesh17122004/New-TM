...
# OSRM base URL — point this at your self-hosted OSRM instance in production
# (see /osrm-setup). Falls back to the public demo server, then to a padded
# haversine estimate, so order creation never hard-fails on a routing outage.
OSRM_URL = os.environ.get("OSRM_URL", "https://router.project-osrm.org").rstrip("/")
OSRM_TIMEOUT_SECONDS = float(os.environ.get("OSRM_TIMEOUT_SECONDS", "3"))
# Straight-line distance underestimates real road distance — used only if
# OSRM is completely unreachable, so delivery fee doesn't silently short the rider.
HAVERSINE_FALLBACK_PADDING = 1.3


def _road_distance_km(rest_lat, rest_lng, cust_lat, cust_lng) -> float:
    """
    Real driving distance from restaurant to customer, in km, via OSRM.
    This is the billed distance for delivery_fee — never straight-line.
    Falls back to a padded haversine estimate if OSRM is unreachable.
    """
    try:
        import requests
        url = (
            f"{OSRM_URL}/route/v1/driving/"
            f"{rest_lng},{rest_lat};{cust_lng},{cust_lat}?overview=false"
        )
        resp = requests.get(url, timeout=OSRM_TIMEOUT_SECONDS)
        data = resp.json()
        if data.get("code") == "Ok" and data.get("routes"):
            return round(data["routes"][0]["distance"] / 1000.0, 2)
        logger.warning("OSRM returned non-Ok response, falling back to haversine: %s", data.get("code"))
    except Exception as exc:
        logger.warning("OSRM request failed (%s), falling back to padded haversine", exc)

    straight = _haversine(rest_lat, rest_lng, cust_lat, cust_lng)
...
FALLBACK_ASSIGN_RADIUS_KM = 15.0  # used only when nobody is available within the normal radius
...
def _offer_to_next_rider(db, order_id):
    ...
    if not eligible:
        # No rider within the normal radius — rather than leaving the order
        # stuck with nobody notified, fall back to the nearest available
        # rider within a wider radius. Better to notify a farther rider than
        # notify no one.
        for rider in all_online:
            rider_id = str(rider["_id"])
            if rider_id in rejected_ids:
                continue
            d = dist(rider)
            if d <= FALLBACK_ASSIGN_RADIUS_KM and _rider_active_count(db, rider_id) < MAX_CONCURRENT_ORDERS:
                eligible.append((rider, d))
        used_fallback_radius = bool(eligible)
    ...
    if eligible:
        ...
        db.orders.update_one(
            {"_id": order_id},
            {
                "$set": {
                    ...
                    "far_offer": used_fallback_radius,
                }
            },
        )
        ...
        try:
            ...
            socketio.emit("delivery_offer", {
                ...
                "offer_timeout": 120,
                "far_offer": used_fallback_radius,  # lets the rider app show "farther than usual" context
        ...
        return {"success": True, "rider_id": next_rider_id, "rider_name": next_rider.get("name", ""), "used_fallback_radius": used_fallback_radius}

    # No eligible riders found even at the wider fallback radius.
    any_nearby = bool(all_online)  # at least someone is online, even if all rejected/busy/too far
...
def place_order():
    ...
    # Calculate ROAD distance from restaurant → customer (never straight-line —
    # this is what the rider is actually paid for and the customer is billed for)
    distance_km = _road_distance_km(rest_lat, rest_lng, cust_lat, cust_lng)

    settings = get_platform_settings(db)
    delivery_fee = _delivery_fee_for_distance(
        distance_km,
