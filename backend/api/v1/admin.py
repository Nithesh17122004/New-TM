...
def save_settings():
    ...
    if "delivery_per_km_fee" in data:
        try:
            update["delivery_per_km_fee"] = max(0.0, float(data["delivery_per_km_fee"]))
        except (TypeError, ValueError):
            return jsonify({"success": False, "message": "Invalid delivery_per_km_fee"}), 400
